                ...
                members.map(member => (
                  <tr key={member.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {MONTHS.map(m => {
                      ...
                      const hasExempt = member.exempts?.includes(m) || false;
                      const hasObligation = member.fee_by_month ? member.fee_by_month[m] !== undefined : true;
                      const isDebtMonth = hasObligation && m <= currentMonth && !hasPaid && !hasExempt;

                      return (
                        <td
                          key={m}
                          title={isDebtMonth ? `${member.name} nợ tháng ${m.split('-')[1]}` : undefined}
                          className={`px-3 py-3 text-center`}
                        >
                          <div className={`mx-auto w-5 h-5 rounded-full flex items-center justify-center ${hasPaid
